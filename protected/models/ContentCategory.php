...
class ContentCategory extends CActiveRecord {
	...
	public function rules() {
		...
		return array (
				...
				array (
						'ordering, published, created_by, modified_by',
						'numerical',
						'integerOnly' => true 
				),
				array (
				...
				array (
						'created_time, modified_time,description',
						'safe' 
				),
				// The following rule is used by search().
				// Please remove those attributes that should not be searched.
				array (
						'id, parent_id, title, alias, description, ordering, published, created_by, created_time, modified_by, modified_time',
						'safe',
						'on' => 'search' 
				) 
		);
	...
	public function getUserName($id) {
		$returnValue = Yii::app ()->db->createCommand ()->select ( 'name' )->from ( '{{user_admin}}' )->where ( 'id=:id', array (
				':id' => $id 
		) )->queryScalar ();
	...
	/**
	 * Retrieves Volume by Category ID.
	 *
	 * @return string.
	 */
	public static function getVolumeNo($id) {
		$value = ContentCategory::model ()->findByAttributes ( array (
				'id' => $id
		) );
	...
	public static function get_category_new($model, $field) {
		$parent1 = Yii::app ()->db->createCommand ()->select ( 'id,parent_id,title' )->from ( '{{content_category}}' )->where ( 'parent_id=0 AND published=1' )->order ( 'parent_id,title' )->queryAll ();
		...
		foreach ( $parent1 as $key => $values1 ) {
			...
			$parent2 = Yii::app ()->db->createCommand ()->select ( 'id,parent_id,title' )->from ( '{{content_category}}' )->where ( 'parent_id=' . $values1 ["id"] . ' AND published=1' )->order ( 'title' )->queryAll ();
			foreach ( $parent2 as $key => $values2 ) {
				$option .= '<option value="' . $values2 ["id"] . '" class="text-success">&rAarr; ' . $values2 ["title"] . '</option>';
				$parent3 = Yii::app ()->db->createCommand ()->select ( 'id,parent_id,title' )->from ( '{{content_category}}' )->where ( 'parent_id=' . $values2 ["id"] . ' AND published=1' )->order ( 'title' )->queryAll ();
				foreach ( $parent3 as $key => $values3 ) {
					$option .= '<option value="' . $values3 ["id"] . '" class="text-danger">&DoubleRightArrow; ' . $values3 ["title"] . '</option>';
					$parent4 = Yii::app ()->db->createCommand ()->select ( 'id,parent_id,title' )->from ( '{{content_category}}' )->where ( 'parent_id=' . $values3 ["id"] . ' AND published=1' )->order ( 'title' )->queryAll ();
	...
	public static function get_category_view($id) {
		$title = ContentCategory::model ()->findByAttributes ( array (
				'id' => $id 
		) );
		$val_top = '';
		$val_middle = '';
		if ($title->parent_id > 0) {
			$parent = ContentCategory::model ()->findByAttributes ( array (
					'id' => $title->parent_id 
			) );
			$val_middle = $parent->title . ' &raquo; ';
			if ($parent->parent_id > 0) {
				$top = ContentCategory::model ()->findByAttributes ( array (
						'id' => $parent->parent_id 
				) );
